var defaults = {
  baseUriParams: <%= _.e(baseUriParameters) %>,
  uriParams: <%= _.e(uriParameters) %>,
...
module.exports = function(overrides) {
  ...
  function xhr(method, path) {
    return function(params) {
      ...
      var url = client.options.endpoint + path,
...
<% } else if (resource.setter) { %>  client.<%= resource.property.join('.') %> = setter(<%= _.e(resource.method) %>);
