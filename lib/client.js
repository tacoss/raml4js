  ...
  function validateParameter(name, value, rules) {
    ...
    validateObject(obj, test);
  }

  var defaults = {
    baseUri: <%= _.e(baseUri) %>,
    baseUriParameters: <%= _.e(baseUriParameters) %>,
    uriParameters: <%= _.e(uriParameters) %>,
    version: <%= _.e(version) %>,
    title: <%= _.e(title) %>
  };
  ...
  function setter(param, rules) {
    return function(value) {
      if (rules) {
        validateParameter(param, value, rules);
      }

      client._uriParameters[param] = value;
...
<% } else if (resource.setter) { %>  client.<%= resource.property.join('.') %> = setter(<%= _.e(resource.method) %>, <%= _.e(resource.uriParameters && resource.uriParameters[resource.method] || null) %>);
