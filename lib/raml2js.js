...
module.exports = function(file, callback) {
  module.exports.read(file, function(err, data) {
    if (err) {
      return callback(err);
    }

    try {
      callback(null, module.exports.client(data));
    } catch (e) {
      callback(e);
    }
  });
...
module.exports.client = function(data) {
  ...
  return client(_.merge({}, {
    resources: flattenSchema(data)
  }, params));
};

module.exports.validate = function(data, logger, callback) {
  if (!callback) {
    callback = logger;
    logger = function(type, obj) {
      console.log('raml2js.log.' + type, obj);
    };
  }

  debug(logger, 'root', data);
  ...
  schemas.forEach(function(params) {
    ...
    if (!uri(params.schema.id)) {
      params.schema.id = params.schema['$schema'] + '/' + params.schema.id;
    }
