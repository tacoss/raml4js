describe 'raml4js', ->
  ...
  it 'should expose a chainable api', ->
    ...
    try
      expect(generated_client.resources.articles.get()).toEqual ['GET', 'http://api.fake.com/v1/articles', data: {}]
      expect(generated_client.resources.articles.articleId(1).get()).toEqual ['GET', 'http://api.fake.com/v1/articles/1', data: {}]
      expect(generated_client.resources.articles.articleId(4).property('body').get()).toEqual ['GET', 'http://api.fake.com/v1/articles/4/body', data: {}]
      expect(generated_client.resources.articles.articleId(13).property('excerpt').set.post()).toEqual ['POST', 'http://api.fake.com/v1/articles/13/excerpt/set', data: {}]
      expect(generated_client.resources.articles.articleId(20).trackback.put({ pong: 'true' })).toEqual ['PUT', 'http://api.fake.com/v1/articles/20/trackback', data: { pong: 'true' }]

      expect(-> generated_client.resources.articles.articleId('123')).toThrow()
      expect(-> generated_client.resources.articles.articleId(100).get(value: '456')).toThrow()
    catch e
